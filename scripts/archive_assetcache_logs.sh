...
# Version: 1.9.1-archive3
#
# Zweck:
# - stoppt den AssetCache-Logger-LaunchDaemon
# - verschiebt sichtbare Altdateien aus /Library/Logs/KommunalBIT
#   in das Archiv unter /Library/Application Support/KommunalBIT/AssetCacheLogger/archive
...
# - keine hart codierten LaunchDaemon-Labels mit Punkt
...
SCRIPT_VER="1${DOT}9${DOT}1-archive3"
...
LOGGER_LABEL="de${DOT}kommunalbit${DOT}assetcachelogger"
LAUNCHD_LABEL="system/${LOGGER_LABEL}"
PLIST_PATH="/Library/LaunchDaemons/${LOGGER_LABEL}${PLIST_EXT}"
...
archive_file() {
  ...
  clean="${base%${CSV_EXT}}"
  dst="${ARCHIVE_DIR}/${clean}_${TS}${CSV_EXT}"

  n=1
  while [[ -e "$dst" ]]; do
    dst="${ARCHIVE_DIR}/${clean}_${TS}_${n}${CSV_EXT}"
    n=$((n + 1))
  done
...
migrate_old_archiv_file() {
  ...
  dst="${ARCHIVE_DIR}/legacyArchiv_${clean}_${TS}${CSV_EXT}"

  n=1
  while [[ -e "$dst" ]]; do
    dst="${ARCHIVE_DIR}/legacyArchiv_${clean}_${TS}_${n}${CSV_EXT}"
    n=$((n + 1))
  done
...
if [[ -d "$LOGDIR" ]]; then
  echo
  echo "--- HU-Dateien"
  find "$LOGDIR" -maxdepth 1 -type f -name "*_AssetCache_Hu_v*${CSV_EXT}" -print0 2>/dev/null | while IFS= read -r -d '' f; do
    archive_file "$f"
  done

  echo
  echo "--- CO-Dateien"
  find "$LOGDIR" -maxdepth 1 -type f -name "*_AssetCache_Co_v*${CSV_EXT}" -print0 2>/dev/null | while IFS= read -r -d '' f; do
    archive_file "$f"
  done

  echo
  echo "--- Legacy-RAW-Dateien"
  find "$LOGDIR" -maxdepth 1 -type f -name "*_AssetCacheRaw_v*${CSV_EXT}" -print0 2>/dev/null | while IFS= read -r -d '' f; do
    archive_file "$f"
  done
else
